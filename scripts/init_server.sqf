...
if (!isNil "init_Svr_Exec") then {
	...
	if !ADF_HC_execute then {(owner ADF_HC1) publicVariableClient "COIN_ambientAirSpawn";};
	...
	if !ADF_HC_execute then {missionNamespace setVariable ["COIN_ao_all", COIN_ao_markers, owner ADF_HC1]};
